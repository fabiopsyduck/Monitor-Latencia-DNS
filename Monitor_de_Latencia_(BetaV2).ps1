# ==============================================================================
# Monitor de Latência DNS (BETA V2)
...
Add-Type -AssemblyName System.Windows.Forms.DataVisualization # <-- LINHA NOVA AQUI
...
# ==============================================================================
# 3. JANELA CUSTOMIZADA PARA ADICIONAR / EDITAR DNS (BLINDADA CONTRA DUPLICATAS)
# ==============================================================================
function Show-DNSDialog {
    param (
        ...
        [array]$ListaExistente = @() # <--- NOVO: Recebe a tabela atual para averiguação
    )
    ...
    # ==========================================================================
    # NOVO MOTOR FISCAL: Checagem de Repetições
    # ==========================================================================
    $btnSalvar.Add_Click({
        ...
        # Varre todos os IPs existentes na tabela
        foreach ($item in $ListaExistente) {
            
            # Se for edição, o script ignora a si mesmo para permitir salvar
            if (-not [string]::IsNullOrEmpty($NomeAtual) -and $item.Nome -eq $NomeAtual) {
                continue
            }
        ...
        if ($duplicado) {
            ...
            return # Trava aqui e não deixa a janela fechar!
        }

        # Se passou na vistoria, fecha com sucesso
        $dlg.DialogResult = "OK"
        $dlg.Close()
    })
...
$form.Text = "Monitor de Latência DNS  (Criador: Fabiopsyduck) (Versão: BETA V2)"
...
# ==============================================================================
# NOVO: GERADOR DO GRÁFICO (PIXEL-PERFECT NA BORDA DO ZERO)
# ==============================================================================
function Show-LatencyChart {
    ...
    # --------------------------------------------------------------------------
    # BLINDAGEM 1: CENTRALIZAÇÃO DE FONTES (Evita vazamento de GDI Handles)
    # --------------------------------------------------------------------------
    ...
    # --------------------------------------------------------------------------
    # BLINDAGEM 2: PROTEÇÃO CONTRA ESCALA DPI (>100%)
    # --------------------------------------------------------------------------
    ...
    # --------------------------------------------------------------------------
    # BLINDAGEM 1 (Fim): Força a liberação da memória de vídeo
    # --------------------------------------------------------------------------
    $fntNormal.Dispose()
    $fntBold.Dispose()
}
...
# ==============================================================================
# NOVO: INTERAÇÃO DA TABELA (BOTÃO ESQUERDO = COPIAR | BOTÃO DIREITO = GRÁFICO)
# ==============================================================================
...
# ==============================================================================
# EVENTOS DOS BOTÕES (COM SINCRONIZAÇÃO E BLINDAGEM CONTRA REPETIÇÕES)
# ==============================================================================
$btnAddIPv4.Add_Click({ 
    $n = Show-DNSDialog -Titulo "Novo IPv4" -ListaExistente @($gridIPv4.DataSource)
    if ($n) { 
...
$btnAddIPv6.Add_Click({ 
    $n = Show-DNSDialog -Titulo "Novo IPv6" -ListaExistente @($gridIPv6.DataSource)
    if ($n) { 
        $n | Add-Member -MemberType NoteProperty -Name "Selecionado" -Value "True"
...
$btnEditIPv4.Add_Click({ 
    if ($gridIPv4.SelectedRows.Count -gt 0) { 
        ...
        $e = Show-DNSDialog -Titulo "Editar IPv4" -NomeAtual $r.Nome -PrimarioAtual $r.DNS_Primario -SecundarioAtual $r.DNS_Secundario -ListaExistente @($gridIPv4.DataSource)
...
$btnEditIPv6.Add_Click({ 
    if ($gridIPv6.SelectedRows.Count -gt 0) { 
        ...
        $e = Show-DNSDialog -Titulo "Editar IPv6" -NomeAtual $r.Nome -PrimarioAtual $r.DNS_Primario -SecundarioAtual $r.DNS_Secundario -ListaExistente @($gridIPv6.DataSource)
...
$ScriptBlockTest = {
    ...
    foreach ($srv in $servidores) {
        ...
        $res = [PSCustomObject]@{
            ...
            TemposBrutos = $tempos.ToArray() # <--- O "bolso secreto" foi adicionado aqui!
        }
...
function Aplicar-FiltrosGrid {
    ...
    if ($null -ne $gridRes.Columns["TemposBrutos"]) { $gridRes.Columns["TemposBrutos"].Visible = $false } # <-- LINHA NOVA AQUI
}
